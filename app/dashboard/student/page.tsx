...
import {
  ...
  STUDENT_HOME_HEADER,
  STUDENT_HOME_STATS,
  type StudentHomeChart,
  type StudentHomeStat,
} from "@/constants/student-home"

const STAT_ICONS: Record<StudentHomeStat["icon"], typeof FileText> = {
  papers: FileText,
  quizzes: ListChecks,
  score: Gauge,
}

const CHART_TONE_CLASSES: Record<StudentHomeChart["tone"], string> = {
  "chart-1": "text-chart-1",
  "chart-2": "text-chart-2",
}

const TREND_TONE_CLASSES: Record<NonNullable<StudentHomeStat["trend"]>["tone"], string> = {
    ...
    <div className="flex flex-col gap-[var(--space-lg)]">
      <section className="flex flex-col gap-[var(--space-2xs)]">
        <h1 className="text-3xl font-semibold text-foreground">
          {STUDENT_HOME_HEADER.title}
        </h1>
        <p className="text-sm text-muted-foreground md:text-base">
          {STUDENT_HOME_HEADER.subtitle}
        </p>
      </section>
        ...
        {STUDENT_HOME_STATS.map((stat) => {
          ...
          return (
            ...
            >
              ...
              {stat.trend ? (
                <div
                  className={`flex items-center gap-[var(--space-xs)] text-xs font-medium ${TREND_TONE_CLASSES[stat.trend.tone]}`}
                >
                  {stat.trend.tone === "positive" ? (
        ...
        {STUDENT_HOME_CHARTS.map((chart) => (
          ...
          >
            <div className="flex items-center justify-between gap-[var(--space-md)]">
              ...
              <Badge
                variant="secondary"
                className="flex items-center gap-[var(--space-2xs)] bg-accent/10 text-accent"
              >
            ...
            <div className="flex flex-col gap-[var(--space-md)]">
              <div className="h-[200px] w-full">
                <svg
                  className={`h-full w-full ${CHART_TONE_CLASSES[chart.tone]}`}
                  fill="none"
                  viewBox="0 0 478 150"
